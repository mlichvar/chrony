...
*maxntsretry* _interval_:::
When the NTS-KE server used with the *nts* option is not responding, or
providing acceptable cookies, *chronyd* will increase the interval between the
attempts exponentially to avoid overloading the NTS-KE server. This option
specifies the maximum interval as a power of 2 in seconds. The default is 19 (6
days), the minimum is 6 (64 seconds), and maximum 24 (6 months).
*maxdelay* _delay_:::
...
device of the PTP clock that should be used as a time source, or the name of
the network interface that is associated with the PTP clock. If the PTP clock
is keeping time in TAI instead of UTC, the *tai* option needs to be added to
the directive, or the current UTC-TAI offset specified by the *offset* option.
...
+
By default, the maximum assumed drift is 100000 ppm, i.e. the adjustment is
...
*precision* _precision_:::
This option specifies the assumed precision of reading of the NIC clock and
maximum error in its tracking relative to the system clock. Larger offsets
cause the tracking to reset and fall back to kernel or daemon timestamps. The
...
(disabled).
+
Support for NTP over PTP (RFC 10030) was added in *chronyd* version 4.9.
It sends NTP-over-PTP messages as PTPv2 Delay_Req messages with the
ORGANIZATION_EXTENSION TLV. It accepts PTPv2 and PTPv2.1 Delay_Req and Sync
messages with the ORGANIZATION_EXTENSION or
