...
// Copyright (C) Miroslav Lichvar  2014-2016, 2020-2026
//
...
The `chrony` source code includes
https://gitlab.com/chrony/chrony/-/raw/master/examples/multi-chronyd.sh[multi-chronyd.sh],
an example bash script that automates the configuration and startup of the
...
On Linux, `chronyd` supports hardware clocks that some NICs have for PTP. They
...
version 4.9, `chronyd` can use PTP as a transport for NTP messages (NTP over
PTP specified in RFC 10030) to enable hardware timestamping on hardware that
can timestamp only PTP
packets. NTP over PTP can be enabled by the `ptpport` directive and setting the
`port` option to the PTP port 319. Since version 4.5, `chronyd` can also apply
delay corrections provided by PTP one-step end-to-end transparent clocks (e.g.
...
When the network has PTP boundary clocks, or transparent clocks that cannot
provide delay corrections in NTP-over-PTP messages, `chronyd` can be combined
with one or more instances of `ptp4l` and `phc2sys` from `linuxptp` to improve
accuracy and resilience against failures by using multiple PTP and/or NTP time
sources simultaneously. The `clock_servo` option can be set to `refclock_sock`
or `ntpshm` to work as a `SOCK` or `SHM` refclock for `chronyd` respectively.
The `timemaster` program included in `linuxptp` can configure and start all the
programs needed for this combined setup.
